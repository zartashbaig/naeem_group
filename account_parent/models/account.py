...
class AccountLevels(models.Model):
    _name = "account.levels"
    _description = 'Account Levels'
...
class AccountAccountType(models.Model):
    ...
    type = fields.Selection(selection_add=[('view','View')], ondelete={'view': lambda recs: recs.write({'type': 'other'})})
    code = fields.Char('Account Code')
    count = fields.Integer('Count NO.', default=0)
    
    _sql_constraints = [('code_unique', 'unique(code)', 'Already exist Code Number')]
...
class AccountAccount(models.Model):
    ...
    levels_id = fields.Many2one('account.levels','Levels')
...
#         if res.user_type_id.code:
#             res.code = int(res.user_type_id.code) + res.user_type_id.count
#             res.user_type_id.count = res.user_type_id.count + 1
#             return res
#         else:
#             raise UserError(
#                     _('Please Check...First you need to define Account Type Code!!!'))
