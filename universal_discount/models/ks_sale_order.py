...
class KsGlobalDiscountSalesInquiry(models.Model):
    ...
    def _amount_all(self):
        res = super(KsGlobalDiscountSalesInquiry, self)._amount_all()
        for rec in self:
    ...
    def _prepare_invoice(self):
        res = super(KsGlobalDiscountSalesInquiry, self)._prepare_invoice()
        for rec in self:
...
class KsGlobalDiscountSalesQuotation(models.Model):
    ...
    @api.depends('order_line.price_total', 'ks_global_discount_rate', 'ks_global_discount_type')
    def _amount_all(self):
        res = super(KsGlobalDiscountSalesQuotation, self)._amount_all()
        for rec in self:
            if not ('ks_global_tax_rate' in rec):
                rec.ks_calculate_discount()
        return res
    ...
    # @api.multi
    def _prepare_invoice(self):
        res = super(KsGlobalDiscountSalesQuotation, self)._prepare_invoice()
        for rec in self:
            res['ks_global_discount_rate'] = rec.ks_global_discount_rate
            res['ks_global_discount_type'] = rec.ks_global_discount_type
        return res
