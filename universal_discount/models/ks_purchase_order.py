...
class KSGlobalDiscountPurchases(models.Model):
    ...
    ks_global_discount_type = fields.Selection([('percent', 'Percentage'), ('amount', 'Amount')],
                                               string='Overall Discount Type', readonly=True,
                                               states={'draft': [('readonly', False)], 'sent': [('readonly', False)]},
                                               default='percent')
    ks_global_discount_rate = fields.Float('Overall Discount', readonly=True,
                                           states={'draft': [('readonly', False)], 'sent': [('readonly', False)]})
    ks_amount_discount = fields.Monetary(string='Overall Discount', readonly=True, compute='_amount_all',
...
class KSGlobalDiscountPurchasesInquiry(models.Model):
    _inherit = "cyb.purchase"
    ...
    @api.depends('order_line.price_total', 'ks_global_discount_type', 'ks_global_discount_rate')
    def _amount_all(self):
        ks_res = super(KSGlobalDiscountPurchasesInquiry, self)._amount_all()
        for rec in self:
            if not ('global_tax_rate' in rec):
                rec.ks_calculate_discount()
        return ks_res
...
class KSGlobalDiscountPurchasesQuotation(models.Model):
    _inherit = "cyb.quotation.purchase"
    ...
    @api.depends('order_line.price_total', 'ks_global_discount_type', 'ks_global_discount_rate')
    def _amount_all(self):
        ks_res = super(KSGlobalDiscountPurchasesQuotation, self)._amount_all()
        for rec in self:
            if not ('global_tax_rate' in rec):
                rec.ks_calculate_discount()
        return ks_res
